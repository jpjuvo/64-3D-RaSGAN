...
parser.add_argument('-dlr','--discriminator_learning_rate', default=0.0005, help ='The discriminator\'s learning rate.', type=float)
...
#Fill logits
logits_d = tf.subtract(D_legit, D_fake)
logits_d2 = tf.subtract(D_eval, D_fake)
logits_g = tf.subtract(D_fake, D_legit)

d_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_d, labels=tf.ones_like(logits_d)))
d_loss2 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_d2, labels=tf.ones_like(logits_d2)))
g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_g, labels=tf.ones_like(logits_g)))
...
g_vars = tl.layers.get_variables_with_name('gen', True, True)   #Train=True (only get trainable variables), Verbose=True (print information)
...
for epoch in range(start, args.epochs):
	...
	for idx in range(0, int(len(files)/batchSize)):
		...
		Train_Dis = ((1.5 * errD_smooth) > errG_smooth)
		d_train_counter += 1
		...
		print("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, TrainD: %s, D_accuracy_low_pass: %.4f" \
					% (epoch, args.epochs, idx, int(len(files)/batchSize) , time.time() - start_time, errD, errG, Train_Dis, D_accuracy_low_pass))
	#saving the model 
